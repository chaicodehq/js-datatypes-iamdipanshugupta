...
export function buildZomatoOrder(cart, coupon) {
  ...
  const Validation = cart.filter(item => item.qty > 0);
  ...
  const couponCode = typeof coupon === "string" ? coupon.toLowerCase() : "";
  ...
  const rawTotal = subtotal + deliveryFee + gst - discount;
  const grandTotal = parseFloat(Math.max(rawTotal ,0).toFixed(2));
